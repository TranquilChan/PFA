...
class MyArgumentParser(object):
    def __init__(self):
        ...
        self.parser.add_argument('--config', "--config", default='config/voc.yaml', type=str, help="config")
...
class Logger(object):
    ...
    def write_without_print(self, message):
        if type(message) != str:
            message = str(message) + '\n'
        with open(self.log_path, mode='a') as f:
            f.write(message)
...
def get_scheduler(optimizer, args):
    ...
    """
    if args.optimizer.lr_policy == 'linear':
        def lambda_rule(epoch):
            lr_l = 1.0 - epoch / float(args.train.epoch + 1)
            return lr_l

        scheduler = lr_scheduler.LambdaLR(optimizer, lr_lambda=lambda_rule)
    elif args.optimizer.lr_policy == 'step':
        step_size = args.train.epoch // 3
        # args.lr_decay_iters
        scheduler = lr_scheduler.StepLR(optimizer, step_size=step_size, gamma=0.1)
    else:
        return NotImplementedError('learning rate policy [%s] is not implemented', args.optimizer.lr_policy)
    return scheduler
...
class Compose(object):
    # Composes segs: segtransform.Compose([segtransform.RandScale([0.5, 2.0]), segtransform.ToTensor()])
